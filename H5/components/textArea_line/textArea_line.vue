  ...
  <section class="area-line-content" :style="contentStyle" :class="className">
    ...
    <div class="input-line-baseStance a-2-bd" :class="baseStanceClass" ref="baseStance">
      <div class="input_line_inp"
        ...
        <svg class="icon icon-14 area-icon" aria-hidden="true">
          <use xlink:href="#icon-a_18"></use>
        </svg>
      </div>
      <!-- 提示信息 -->
      <label class="input-line-prompt"
        :class="promptClass"
        @click="promptClick">{{ promptText }}</label>
      <!-- 选中时横线 -->
    ...
    <div class="input-line-errorStence" v-if="errorHave">
      <!-- 错误信息容器 -->
      <p class="input-line-error b-6-cl" v-if="errorFlag && !isFocus">{{ errorText }}</p>
      <!-- 警示文案 -->
      <p class="input-line-warning" v-else-if="warningFlag">{{ warningText }}</p>
    </div>
      <!-- 底部提示框 -->
